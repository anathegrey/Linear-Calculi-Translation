...
-- OPERATIONAL SEMANTICS
  ...
  eval :: WStore -> Int -> WTerm -> (WStore, WTerm)
  eval s n (WPair q t1 t2) = let x = "a" ++ (show n)
                             in (Map.insert x (QValue q (RPair t1 t2)) s, WVar x)
  eval s n (WSplit (WVar x) y z t) = let Just (QValue q (RPair t1 t2)) = Map.lookup x s
                                         s'  = updateStore s q x
                                         t'  = subst (y, t1) t
                                         t'' = subst (z, t2) t'
                                     in (s', t'')
